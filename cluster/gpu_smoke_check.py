"""Fast GPU smoke check run after installation and before the equivalence suite.

Jobs 38938670 and 38938725 reached CUDA on V100 nodes but failed inside
rapids-singlecell preprocessing kernels with `named symbol not found`, because the
pinned wheel does not carry that architecture. Job 38938173 saw its allocated A100
in `nvidia-smi` while CUDA reported `cudaErrorNoDevice`. Both classes of failure are
node- or wheel-specific rather than scientific, and both waste the whole allocation
if they are only discovered part-way through the suite.
