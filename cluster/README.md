...
1. stages both repositories from shared storage to `/localscratch/$USER`;
...
4. installs a staged sibling `rapids_singlecell` checkout when available;
...
```bash
sbatch --export=ALL,REPRO_SOURCE=/lustre/groups/ml01/workspace/$USER/rapids-singlecell-reproducibility,RSC_SOURCE=/lustre/groups/ml01/workspace/$USER/rapids_singlecell cluster/run_comparisons.sbatch

# Use the rapids-singlecell release pinned in uv.lock instead of a source checkout:
sbatch --export=ALL,RSC_SOURCE= cluster/run_comparisons.sbatch
