...
metrics.append(lower("bbknn.connectivity_jaccard", graph_overlap(reference.obsp["connectivities"], candidate.obsp["connectivities"]), 0.85))
...
metrics.extend([
    lower("scrublet.score_correlation", pearson(reference.obs["doublet_score"], candidate.obs["doublet_score"]), 0.95),
    lower("scrublet.call_agreement", np.mean(reference.obs["predicted_doublet"].to_numpy() == candidate.obs["predicted_doublet"].to_numpy()), 0.95),
])

reference_sim = sc.pp.scrublet_simulate_doublets(counts, sim_doublet_ratio=1.0, random_seed=0)
candidate_sim = rsc.pp.scrublet_simulate_doublets(counts, sim_doublet_ratio=1.0, random_seed=0)
...
metrics.extend([
    upper("scrublet_simulate_doublets.mean_library_size_relative_error", abs(ref_totals.mean() - gpu_totals.mean()) / ref_totals.mean(), 0.02),
    upper("scrublet_simulate_doublets.std_library_size_relative_error", abs(ref_totals.std() - gpu_totals.std()) / ref_totals.std(), 0.05),
])
