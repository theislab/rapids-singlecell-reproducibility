...
for label, obj in (("cpu", reference), ("gpu", candidate)):
    metrics.append(
        lower(
            f"tsne.{label}.trustworthiness",
            trustworthiness(adata.obsm["X_pca"], obj.obsm["X_tsne"], n_neighbors=15),
            0.90,
        )
    )
metrics.append(
    lower("tsne.cross_embedding_knn_overlap", knn_overlap(reference.obsm["X_tsne"], candidate.obsm["X_tsne"]), 0.45)
)
...
metrics.append(
    lower(
        "embedding_density.pearson_correlation",
        pearson(reference.obs["umap_density"], candidate.obs["umap_density"]),
        0.999,
    )
)
