...
rsc.ptg.Mixscape().perturbation_signature(gpu, pert_key="gene_target", control="NT", n_neighbors=5, knn_algorithm="brute")
...
metrics.extend([
    lower("mixscape.global_class_agreement", np.mean(cpu.obs["mixscape_class_global"].to_numpy() == gpu.obs["mixscape_class_global"].to_numpy()), 0.95),
    lower("mixscape.p_ko_correlation", pearson(cpu.obs["mixscape_class_p_ko"], gpu.obs["mixscape_class_p_ko"]), 0.95),
])
...
metrics.append(lower("mixscape.lda_abs_correlation", abs(pearson(cpu.uns["mixscape_lda"], gpu.uns["mixscape_lda"])), 0.95))
