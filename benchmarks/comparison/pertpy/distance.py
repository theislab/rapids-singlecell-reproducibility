...
for name in metric_names:
    ...
    tolerance = 1e-4 if name in {"wasserstein", "edistance"} else 1e-5
    metrics.append(upper(f"{name}.pairwise_max_abs_error", error, tolerance))
    metrics.append(lower(f"{name}.pairwise_correlation", np.corrcoef(reference.to_numpy().ravel(), candidate.to_numpy().ravel())[0, 1], 0.9999))
